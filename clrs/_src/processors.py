...
class PGN(Processor):
    ...
    def __call__(  # pytype: disable=signature-mismatch  # numpy-scalars
        ...
        gate4 = hk.Linear(self.out_size, name='bank_gate5')
        use_bank = False
        if use_bank:
            ...
            if self.use_triplets:
                # Triplet messages, as done by Dudzik and Velickovic (2022)
                triplets = get_triplet_msgs(z, edge_fts, graph_fts, self.nb_triplet_fts)  # b n n n h
            ...
            if self.gated:  # 默认True
                ...
                if self.use_ln:
                    ln = hk.LayerNorm(axis=-1, create_scale=True, create_offset=True)
                    ret = ln(ret)
                gate = jax.nn.sigmoid(gate3(jax.nn.relu(gate1(z) + gate2(msgs) + msg3_gate)))
...
class MPNN(PGN):
    ...
    def __call__(self, node_fts: _Array, edge_fts: _Array, graph_fts: _Array,
                 adj_mat: _Array, hidden: _Array, Att=None, bank_list=None, use_bank=None, **unused_kwargs) -> _Array:
        adj_mat = jnp.ones_like(adj_mat)
        return super().__call__(node_fts, edge_fts, graph_fts, adj_mat, hidden, Att, bank_list, use_bank)
