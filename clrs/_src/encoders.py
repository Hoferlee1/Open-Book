...
def step_accum_edge_fts(encoders, dp: _DataPoint, edge_fts: _Array) -> _Array:
  ...
  if dp.location == _Location.NODE and dp.type_ in [_Type.POINTER,
    ...
    if dp.type_ == _Type.POINTER:
      ...
      edge_fts += jnp.mean(encoding, axis=2) + jnp.mean(encoding_2, axis=3)
    else:
      edge_fts += encoding
...
def accum_edge_fts_step0(encoders, dp: _DataPoint, edge_fts: _Array) -> _Array:
  """Encodes and accumulates edge features."""
  if dp.location == _Location.NODE and dp.type_ in [_Type.POINTER,
                                                    _Type.PERMUTATION_POINTER]:
    encoding = _encode_inputs_step0(encoders, dp)
    edge_fts += encoding
...
def accum_node_fts_step0(encoders, dp: _DataPoint, node_fts: _Array) -> _Array:
  """Encodes and accumulates node features."""
  is_pointer = (dp.type_ in [_Type.POINTER, _Type.PERMUTATION_POINTER])
  if ((dp.location == _Location.NODE and not is_pointer) or
      (dp.location == _Location.GRAPH and dp.type_ == _Type.POINTER)):
...
def accum_graph_fts_step0(encoders, dp: _DataPoint,
                    graph_fts: _Array) -> _Array:
  """Encodes and accumulates graph features."""
  if dp.location == _Location.GRAPH and dp.type_ != _Type.POINTER:
    encoding = _encode_inputs_step0(encoders, dp)
...
def _encode_inputs_step0(encoders, dp: _DataPoint) -> _Array:
  if dp.type_ == _Type.CATEGORICAL:
    encoding = encoders[0](dp.data[0])
  else:
    encoding = encoders[0](jnp.expand_dims(dp.data[0], -1))
  return encoding